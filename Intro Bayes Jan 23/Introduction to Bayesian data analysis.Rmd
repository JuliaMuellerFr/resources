...
![Here is what it should not look like](img/bad_pp_check.PNG)
...
Is the intercept (reading times in the no training condition) above 300?
```{r}
hypothesis(reading_mld_reg, "Intercept > 300")
```
...
Extracting posterior distributions and visualising them to compare:
```{r}

```
Note that we could also look at the model summaries and compare them.
...
- LOO cross-validation: fit model on all data points except one, then evaluate its prediction for that data point, then rinse and repeat
- establishes predictive error on unseen data points
```{r}
loo(m1, m2)
```
