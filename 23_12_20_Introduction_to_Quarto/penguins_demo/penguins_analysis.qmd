...
execute:
  echo: false # code isn't shown in the document
...
![Explaining bill dimensions](img/culmen_depth.png){#fig-bill width=10cm fig-align="left"}
...
* bill lengths and depths
  + the average bill length is `r mean(penguins$bill_length_mm)` mm 
  + the average bill depth is `r mean(penguins$bill_depth_mm)` mm
...
Penguins on the island `r penguins_bill_length %>% slice_min(order_by = m, n = 1) %>% select(island)` have the smallest average bill length.

* body mass
  + the average weight is `r mean(penguins$body_mass_g)` g


## Separate per species

@tbl-bill shows bill lengths for each of the three species of penguin in the data.
...
penguins %>% 
  group_by(species) %>% 
  summarise("average bill length" = mean(bill_length_mm),
            "variance" = var(bill_length_mm),
            "std. deviation" = sd(bill_length_mm)) %>% 
...
penguins_mdl_tidy <- tidy(penguins_mdl) %>% 
  ...
  mutate(across(where(is.numeric), \(x) round(x, digits = 4)), # round numeric variables to 4 decimals
         p.value = as.character(p.value), # this can cause p-values to show up as 0.0000
...
The model output is shown in @tbl-penguinsMdl. Only the slope for body mass is significant at p < 0.05 (b = `r penguins_mdl_tidy %>% filter(term == "body_mass_g") %>% select(estimate)`, SE = `r penguins_mdl_tidy %>% filter(term == "body_mass_g") %>% select('standard error')`).

<br>

## Model predictions

The model predictions are visualised in @fig-MdlPred.
