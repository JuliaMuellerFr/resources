...
description: Creating documents and slides with Quarto
...
## Who am I? {.smaller}
- working on PhD in English linguistics (re: the interaction between a person's first and second language during language processing)  
- co-organiser of R-Ladies Freiburg and Julia Gender Inclusive  
- favourite tasks in R: creating elaborate graphics, fitting and evaluating mixed effects regression models  
- currently learning: Bayesian methods
- really into grandmotherly activities: cooking, embroidery, sewing clothes  
- total cat lady (stick around for pictures!)  
...
animal_plot <- ggplot(data = animal_rescues) +
  aes(x = hours, y = cost, colour = animal_group)  +
  geom_point() +
  geom_smooth(method = "lm") +
  labs(x = "hours", 
       y = "cost in pounds",
       colour = "animal", 
       title = "Animal rescue costs in London",
...
- all materials available on my Github  
- show features on these slides ("Welcome to Quarto.html")
    - will open in your browser  
- invite you to try them by working on the file "Quarto demonstration - START HERE.qmd"
    - will open in RStudio  
...
#### The YAML
- The YAML header is the first section  
- it's at the top of the document, framed by three hyphens  
...
- insert a new one by clicking on the green drop-down menu at the top in RStudio  
...
How the output and the code itself shows up in the knitted document can also be modified by using these options at the top of a code chunk:  
...
- `#| eval: false` to ensure code is not run  

---

### Code folding {.smaller}
Code chunks can also be "folded", i.e., hidden at first, with `#| code-fold: true`  
The user can click on a "Code" button to reveal the code block  
...
- Inline code is also possible, i.e., in the text, write:  

```{r}
#| eval: false
We find an average distance of `r mean(df$distance)` m.
```

- Whatever code is included in the backticks will be run and the output will be shown in the text.  
- This is useful for printing measures such as the mean or elements of a model summary without having to copy-paste numbers.  
...
This code assumes that this file is in the same folder as the Quarto document. If it's in a subfolder, use `(sub/quarto_logo.png)`
...
- The caption will be displayed underneath the graph  
- Graphs will automatically be numbered, and you can use the label you set for in-text references  
...
#| echo: true
#| label: fig-animals
#| fig-cap: Animal rescues
animal_plot
```

@fig-animals shows animal rescue data.
...
#### Try it
- Add a label and a caption to the first penguin graph.  
- Underneath, write a sentence that references it and the drawing that explains how bill length and depth are measured.  
...
## References {.smaller}
In many works such as academic articles, you'll need to have references (usually, the author and year a text that you're referring to/quoting from) in the body of the text, plus a bibliography section at the end of the text. You can insert these into Quarto.

It's easiest if you use reference management software such as Zotero. Those programs can export the citations you'll need into a .bib file. Put that in the same folder as your Markdown document.
...
#### Try it
- In the YAML, add the penguins.bib file as the bibliography for the document and link citations.  
- Cange the citation style (either using the provided file apa-single-spaced.csl or downloading another csl file).  
...
#| eval: false
:::{.callout-caution}
### This is the callout's title
This is the callout's text
:::
```
...
Then, when you create a new document, select Output: pdf
...
To automatically make a newly rendered Word document conform to a template (in terms of, e.g., page dimensions, font, font sizes, margins, settings for headlines...):  

- create a simple Quarto document that contains all the features you need (e.g., all headline levels you'll use)  
- render it to Word  
- open it and make changes as you wish  
- in your main Quarto document's YAML, refer to your reference document by writing `reference-doc: custom-reference-doc.docx`
...
Get in touch with me via [Twitter](https://twitter.com/JuliaMuellerFr)
