...
- favourite tasks in R: creating elaborate graphics (spending hours on picking a colour scheme and font), fitting and evaluating mixed effects regression models  
...
- thorough overview of the main features  
- description of lots of features for html documents (many of which transfer to Word and pdf)  
- ...and for html slides  
- a quicker look at Word and pdf documents  
- language and environment: R and RStudio  
...
### Inline code
Inline code is also possible, i.e., in the text, write `r `. Whatever code is included in the backticks will be run and the output will be shown in the text.  
This is useful for printing measures such as the mean or elements of a model summary without having to copy-paste numbers (especially if these numbers might still change). 
...
dat <- flametree_grow(time = 4, trees = 4)
...
- in your main Quarto document's YAML, refer to your reference document by writing:  
```{r}
#| eval: false
format:
  docx:
