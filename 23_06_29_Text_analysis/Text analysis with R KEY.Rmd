...
author: "Julia Müller"
date: "Data Café Kiel, June 29 2023"
...
* Currently in a legal battle in Germany
-> impossible to download via the website
...
### TRY IT
Use the `unnest_tokens()` function on the Shakespeare texts.
```{r}
...
fairytales_freq %>% 
  ...
  slice_max(order_by = n, n = 10) %>%  # find the ten words with the highest frequencies
  ggplot() +
...
alice_bigram_tfidf %>%
  ...
  ggplot(aes(reorder_within(bigram, tf_idf, chapter), tf_idf, fill = chapter)) +
  geom_col(show.legend = FALSE) +
  theme_light() +
  labs(x = NULL, y = "tf-idf") +
  facet_wrap(~ chapter, scales = "free") +
